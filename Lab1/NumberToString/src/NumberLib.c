...
static char* digitToWord(int digit) {
  switch (digit) {
    case 0:
      return "zero";
    case 1:
...
static int digitToWordSize(int digit) {
  switch (digit) {
    case 0:
      return 4;
    case 1:
...
static int popDigit(int* number) {
  int pop = *number % 10;
  *number = *number / 10;
  return pop;
}
...
static void copy(char* str, char* dest) {
  for (int i = 0; str[i] != '\0'; i++) dest[i] = str[i];
}
...
char* NumToStr(int number) {
  ...
  char* dest =
      (char*)malloc((count * MAX_WORD_SIZE_LIMIT + count + 1) * sizeof(char));
